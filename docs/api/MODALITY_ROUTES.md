...
### Base Response Models (from `src/ues/api/models.py`)
...
### Common Request Models (from `src/ues/api/models.py`)
...
### Utility Functions (from `src/ues/api/utils.py`)
...
Create `src/ues/api/routes/{modality}.py` with this structure:
...
**Route Files**: `src/ues/api/routes/<modality>.py` (lowercase, singular)
...
- Actions: Use shared `ModalityActionResponse` (defined in `src/ues/api/models.py`)
...
2. **Use Shared Response Types**: `ModalityActionResponse` is defined in `src/ues/api/models.py`
3. **Use Helper Utilities**: `create_immediate_event()` from `src/ues/api/utils.py` handles event creation
